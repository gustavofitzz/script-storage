...
Core.Modules.File																				=				function()
	...
	function File.Functions.FormateName(FileName)
		...
		FormatedFileName																		=					string.gsub(FormatedFileName, 'GameName', Core.Services.MarketplaceService:GetProductInfo(game.PlaceId).Name)
		FormatedFileName																		=					string.gsub(FormatedFileName, 'Game Name', Core.Services.MarketplaceService:GetProductInfo(game.PlaceId).Name)
	...
	function File.Functions.Read(FilePath)
		local FileFound, Error																	=					pcall(function()
			readfile(FilePath)
		end)
	...
	function File.Functions.Write(FilePath, Contents)
	...
	function File.Functions.Save(FilePath, Contents)
	...
	function File.Functions.Clear(FilePath)
		File.Write(FilePath, '');
		local FileName, FileExtention															=					File.Functions.FormateName(FilePath);
		
		Core.Services.StarterGui:SetCore('SendNotification', 
			{
...
Core.Modules.RbxUtility																			=				function()
	...
	RbxUtility.Functions.Create 																=				LoadLibrary('RbxUtility').Create;
...
Core.Modules.DescendantStorage																	=				function()
	...
	function DescendantStorage.Functions.Registar(object)
...
Core.Modules.ArgumentCompiler																	=				function()
	--[[ | ArgumentCompiler | ]]--
	...
	function ArgumentCompiler.Functions.PathFormat(Obj)
		local Path																				=					
			{
				[1]																				=					
				{
			...
			if not Core.Services[Obj.ClassName] then
				...
				if Path[#Path].ClassName			==					'Nil' then
					...
					if string.sub(FullName, 42, 41 + #Core.Services.Players.LocalPlayer.Name) == Core.Services.Players.LocalPlayer.Name then
					...
					elseif string.sub(FullName, 44, 43 + #Core.Services.Players.LocalPlayer.Name) == Core.Services.Players.LocalPlayer.Name then
	...
	function ArgumentCompiler.Functions.TableCompiler(tbl, space, not_clever)
		local lua_keyword;
			...
			writeit = function (t,oldindent,indent)
				...
				elseif tp == 'ColorSequence' then
						putln(quote_if_necessary('ColorSequence.new(' .. ts:gsub("^%s*(.-)%s*$", "%1"):gsub(" ", ", ") .. ')')..';')
				elseif tp == 'NumberRange' then
						putln(quote_if_necessary('NumberRange.new(' .. ts:gsub("^%s*(.-)%s*$", "%1"):gsub(" ", ", ") .. ')')..';')
	...
	function ArgumentCompiler.Functions.Decompile(obj)
...
function Core.Functions:Start()
	...
	GameMeta.__index, GameMeta.__namecall = function(Object, key)
		...
		return function(_, ...)
			...
			TempFormat.Remote																	=					tostring( 'Remote      ||     ' .. Object.Name .. ' Has been ' .. string.gsub(ClassMethods[Object.ClassName], 'Server', 'd') .. '!\n' );
			TempFormat.Path																		=					tostring( 'Path        ||     ' .. Core.Modules.ArgumentCompiler().PathFormat(Object) .. ':' .. ClassMethods[Object.ClassName] ..'();\n' );
			TempFormat.Arguments																=					tostring( 'Arguments   ||     ' .. '(' .. Core.Modules.ArgumentCompiler().Decompile(allPassed) .. ');\n' );
			TempFormat.Return																	=					tostring( 'Return      ||     ' .. '(' .. Core.Modules.ArgumentCompiler().Decompile(returnValues) .. ');\n' );
			TempFormat.Example																	=					tostring( 'Example     ||     ' .. Core.Modules.ArgumentCompiler().PathFormat(Object) .. ':' .. ClassMethods[Object.ClassName] ..'(' .. Core.Modules.ArgumentCompiler().Decompile(allPassed) .. ');' .. '\n\n\n\n' );
			TempFormat.Indent																	=					tostring( '_______________________________________________________________________________________________________________________________________________________' .. '\n\n\n\n\n');
			
			if Passed then
				returnValues = Data
			end
			
			if not Core.Temp[TempFormat.Arguments] then
				Core.Temp[TempFormat.Arguments]													=					true;
			else
				return unpack(returnValues);
			end
			
			if Core.Settings.Recorder.Enabled and Core.Settings.Remotes.Ignored[Object] and Core.Settings.Logs.Save and #TempFormat.Arguments <= Core.Settings.Logs.MaxLogSize and #TempFormat.Return <= Core.Settings.Logs.MaxLogSize then
				pcall(function()
